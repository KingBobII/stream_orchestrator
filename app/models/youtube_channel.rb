class YoutubeChannel < ApplicationRecord
  ...
  before_validation :assign_stream_access_key, on: :create

  scope :active, -> { where(status: "active") }
  scope :inactive, -> { where(status: "inactive") }
  scope :owned_by, ->(user) { where(owner_id: user.id) }
  scope :visible_to, ->(user) { where(stream_access_key: user.stream_access_key) }
  ...
  def assign_stream_access_key
    self.stream_access_key ||= owner&.stream_access_key
  end
