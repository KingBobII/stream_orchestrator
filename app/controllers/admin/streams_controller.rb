...
module Admin
  class StreamsController < Admin::BaseController
    ...
    def create
      ...
      if @stream.save
        queue_youtube_sync(@stream) if @stream.syncable_to_youtube?
        redirect_to admin_stream_path(@stream), notice: @stream.syncable_to_youtube? ? "Stream created and queued for YouTube sync." : "Stream created"
      else
        render :new, status: :unprocessable_entity
      end
    end
    ...
    def update
      if @stream.update(stream_params)
        queue_youtube_sync(@stream) if @stream.syncable_to_youtube?
        redirect_to admin_stream_path(@stream), notice: @stream.syncable_to_youtube? ? "Stream updated and queued for YouTube sync." : "Stream updated"
      else
        render :edit, status: :unprocessable_entity
      end
    end
